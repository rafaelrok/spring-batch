...
public class FuncionarioReaderJdbcConfig {
	...
	JdbcCursorItemReader<Funcionario> funcionarioReaderJdbc(
		...
		return new JdbcCursorItemReaderBuilder<Funcionario>()
				...
				.sql(
						"select *, MONTH(data) as mes, YEAR(data) as ano from funcionario left join registro_ponto on (matricula = funcionario_id and (data is null or MONTH(data)=MONTH(now()) and YEAR(data)=YEAR(now()))) order by matricula")
